...
## 达人探店
**发布探店笔记**
- 数据库认知补充：实际生产中不会加外键，这会给每次操作进行验证，会大大降低性能
- MultiPartFile 是用于处理 SpringBoot 文件上传的接口
- 需要实现查看探店笔记的接口

**点赞功能**
- 一个用户只能点赞一次，再次点击则取消点赞
- 如果当前用户已经点赞，则点赞按钮高亮显示(需要判断Blog类的isLike属性)
- 下面是实现步骤。自己实现的时候，注意把 redis 写为 lua 脚本即可
![img_5.png](img_5.png)

**点赞排行榜**
- 把笔记点赞的人显示出来，比如最早点赞TOP5，形成排行榜
- 需要选择 set，并且有序，所以希望使用排行榜的 zset (SortedSet)，它可以根据 score 值排序
- zscore 可以查询元素的同时返回分数，如果元素不存在，则返回null
- 如果按照时间戳(zscore)查，则 zrange z1 0 4 即可查到前5名
- Redis Nil bulk reply and Nil multi bulk reply -> Lua false boolean type
- BUG：注意 redis 的 (nil) 是 lua 的 false ！
- BUG：之所以检查 zscore 的时候频繁出错，是因为之前使用 set 表示的键值对没有被删除，导致 key 的类型就不是 zset ！
- NOTE：注意 order by field(id, 1, 5) 这种给定查询顺序的使用方式
- 下面是一种 MyBatis 非常神奇的注解写法。当然，还是不太推荐这种写法，如果手写 SQL，还是要写入 mapper.xml 文件
```angular2html
...
public interface YourMapper {
    @Select({
        "SELECT * ",
        "FROM your_table ",
        "WHERE id IN (5, 1, 4, 3, 2) ",
        "ORDER BY FIELD(id, 5, 1, 4, 3, 2)"
    })
    List<YourEntity> selectByIdsInOrder();
}
```

## 好友关注
- 实现：①关注和取关接口，②判断是否关注的接口
- 你有一张表，记录了用户之间的关注关系 tb_follow
- 共同关注：集合取交集

**Feed流**
- 关注推送：(投喂)：为用户提供沉浸式体验，通过无限下拉刷新获取新信息
- TimeLine：不做内容筛选，按照简单的内容发布时间排序，常用于好友或者关注，例如朋友圈。
- 智能排序：利用智能算法屏蔽掉违规、用户不感兴趣的内容。推送用户感兴趣的信息吸引用户。
- Feed流的实现方案：拉模式(发送者发送消息存在发件箱，用户获取消息需要从发件箱获取；内存占用小，延迟高；用的比较少)
- Feed流的实现方案：推模式/写扩散(比如发送者发送消息，那么会推送到所有粉丝的收件箱，粉丝打开收件箱时不用再从发件箱获取信息了)缺点：内存占用较高，延迟低；
场景：用户量少(千万级别用户以下)，没有大V[本项目采用推模式]
- Feed流的实现方案：推拉结合模式/读写混合(普通人发消息使用推模式，普通人没有发件箱；大V：只有活跃粉丝采用推模式、普通粉丝采用拉模式，大V有发件箱)
推拉结合：(比如微博、字节等大平台)

**Feed流：基于推模式实现关注推送功能**
- 修改新增探店笔记的业务，在保存 blog 到数据库的同时，推送到粉丝的收件箱[使用 zset，记录所有 blog id]
- 收件箱满足可以根据时间戳排序，必须用 redis 的数据结构实现
- 查询收件箱数据时，可以使用分页查询
- 如何实现收件箱？用 redis，且按照时间戳排序，则选择 list 和 zset 之一
- Feed流中的数据会不断更新，所以数据的角标也在变化，因此不能采用传统的分页模式 
- (例如，在传统IPAGE的分页模式下：t1 读取 10 ~ 6；t2 发布了新的 feed11；t3 读取第二页，6 ~ 2，则重复读取了6) 
- 需要采用滚动分页模式：由于数据有序，可以记录每次查询的最后一条，下次从该位置开始查。
- 第一次假设 lastId 是无穷大，读取10~6，第二次查的时候，lastId = 6，所以从5开始查询。它不受到数据角标的变化。
- list 不支持滚动分页，而 sorted set 支持范围查询，按照时间戳排序，每次记录的 lastId 是时间戳即可
- 使用 zset 的 ZREVRANGEBYSCORE 实现滚动分页查询，时间戳从高到低，由新到旧。假设每页有三个记录。每次查询需要记录上次查询的最后一个ID值。
- 功能是自行实现的，bug标注在了代码中。

## 附近商铺查询
- geo 允许存储地理坐标信息
- 使用 help@geo 获取关于坐标的帮助
- geo 底层使用 sorted set。 value 是你坐标位置的名称，score 是转化后的经纬度
- gadd g1 可以对应多个location !
- geoadd g1 116.37 39.86 "beijing nan"
- geoadd g1 114.37 37.86 "beijing xi"
- geodist g1 "beijing nan" "beijing xi" m 计算二者空间距离，以 m 为单位
- 附近商户搜索，分类，比如美食一个key，KTV一个key，然后 value 就是商铺，值就是经纬度。
- 导入店铺信息的时候，将它的经纬度放入 redis 中，然后再利用 geo 结构查询即可。
- 实现：没必要用 geo，直接在 MySQL 中按照经纬度的距离排序即可

## 用户签到
- 可以借助数据库表实现签到，核心结构 [用户id, 签到的年, 签到的月， 签到的日期， 是否补签]
- 如果有 1000万用户，平均每人每年签到 10次，则改表一年的数据量是 1 亿条
- 可以使用 bitmap 统计签到情况。比如长度31 bit，1表示当日签到，0表示当日未签到
- redis 利用 string 类型数据结构实现 bitmap
- setbit key offset value 对某个 bit 位置0或1
- getbit key offset
- bitfield bm1 get u2 6 按照范围取比特位，u2 6 表示偏移是6，读2个bit，u表示返回的数值按照无符号数解析(注意：i是有符号数)
- bitpos bm1 0 查找第一个0出现的位置(从左往右)， bitpos bm1 1 查找第一个1出现的位置(从右往左)
- 签到以月为统计单位，每个用户每个月的签到放到一个 bitmap(作为一个key)。

**统计连续签到**
- 这个月签到了几次？这个月连续签到了几次？可以用动态规划，统计最大连续的1即可。